...
export function resolveReleaseContext({ eventName, ref, refName, sha, requestedVersion, cargoToml, contract }) {
  ...
  invariant(!contract.channels?.some(({ id }) => id === "self-update"), "self-update has been removed");
  // Preserve the immutable release-manifest v1 shape without enabling an updater.
  const minimumSupportedCliVersion = version;
